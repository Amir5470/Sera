...
const callGemini = async (base64: string, prompt: string) => {
  ...
  // Send the prompt plus a short instruction to parse into JSON; include the base64 as an inline mention
  const body = {
    prompt: {
      messages: [
        {
    ...
    // Use a conservative token limit
    maxOutputTokens: 800,
  };
  ...
  // Try to find a JSON array in the text output
  const out =
...
export const extractClasses = async (base64: string) => {
  ...
  if (AI_PROVIDER === "gemini") return callGemini(base64, prompt);
  return callAnthropic(base64, prompt);
};
